...
/*
 * Added by kevin_zhaotk for multi-bundle handing
 */
+ (NSURL *)bundleURLByDeploymentKey:(NSString *) deploymentKey {
    
    NSError *error;
    return [CodePushPackage getPackageURLByDeploymentKey:deploymentKey error:&error];
}
                         ...
                         bundle:(NSBundle *)resourceBundle
{
    ...
    NSLog(@"--->bundleURLForResource  packageFile: %@\n", packageFile);
 ...
 * the bridge. This isn't meant to be called directly.
 */
- (void)loadBundle:(NSString *) deploymentKey
{
    // This needs to be async dispatched because the bridge is not set on init
    // when the app first starts, therefore rollbacks will not take effect.
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([CodePush isUsingTestConfiguration] || ![super.bridge.bundleURL.scheme hasPrefix:@"http"]) {
//            [super.bridge setValue:[CodePush bundleURL] forKey:@"bundleURL"];
            [super.bridge setValue:[CodePush bundleURLByDeploymentKey:deploymentKey] forKey:@"bundleURL"];
        }
                    ...
                    rejecter:(RCTPromiseRejectBlock)reject)
{
    ...
    if (!onlyIfUpdateIsPending || [[self class] isPendingUpdate:nil]) {
        if (!deploymentKey) {
            NSLog(@"--->deploymentKey is null, load default bundle");
            [self loadBundle];
        } else {
            NSLog(@"--->deploymentKey: %@\n", deploymentKey);
            [self loadBundle:deploymentKey];
        }
        resolve(@(YES));
        return;
    }
