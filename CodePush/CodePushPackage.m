...
/*
 * get updated bundle URL corespond to specified deploymentkey
 * this API is recommended when restartApp
 */
+ (NSURL *) getPackageURLByDeploymentKey: (NSString *) deploymentKey error:(NSError **) error {
    
    NSString *bundlePath = [CodePushPackage getPackageBundlePathByDeploymentKey:deploymentKey error:error];
    if (!bundlePath) {
        return nil;
    }
...
/*
 * get Current package bundle path by deploymentKey
 * Add by kevin_zhaotk
 */
+ (NSString *) getPackageBundlePathByDeploymentKey:(NSString *)deploymentKey error:(NSError **)error {
    
    
    NSDictionary *info = [self getCurrentPackageInfo:error];
    if (!info || deploymentKey == nil) {
        return nil;
    }
    NSString *currentPackageKey = [@"currentPackage-" stringByAppendingString:deploymentKey];
    NSString *hash = info[currentPackageKey];
    
    NSString *packageFolder = [self getPackageFolderPath:hash];
    
    NSDictionary *currentPackage = [CodePushPackage getPackage:hash error:error];
    NSString *relativeBundlePath = [currentPackage objectForKey:RelativeBundlePathKey];
...
+ (NSString *)getCurrentPackageHash:(NSError **)error
{
    ...
    /* Modification: multi bundles support - by kevin_zhaotk */
//    return info[@"currentPackage"];
    NSString *currentPackage = [@"currentPackage-" stringByAppendingString:[CodePushConfig current].deploymentKey];
    return info[currentPackage];
}
                 ...
                 error:(NSError **)error
{
    ...
    /* Modification: multi bundles support - by kevin_zhaotk */
    NSString *currentPackage = [@"currentPackage-" stringByAppendingString:[updatePackage valueForKey:@"deploymentKey"]];
    NSString *previousPackage = [@"previousPackage-" stringByAppendingString:[updatePackage valueForKey:@"deploymentKey"]];
...
+ (void)rollbackPackage
{
    ...
    /* Modification: multi bundles support - by kevin_zhaotk */
    NSString *currentPackage = [@"currentPackage-" stringByAppendingString:[CodePushConfig current].deploymentKey];
    NSString *previousPackage = [@"previousPackage-" stringByAppendingString:[CodePushConfig current].deploymentKey];
